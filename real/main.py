...
render_preset_fields = config.get(
    "preset_fields", ["hr_enabled", "hr_scale", "hr_upscaler", "denoising_strength"]
)
cn_preset_fields = config.get(
    "cn_preset_fields",
    ["controlnet_model", "controlnet_weight", "controlnet_guidance_end"],
)
...
def update_size_thread(**kwargs):
    ...
    if round(interface_width / interface_height * 100) != round(
        init_width * (1 if img2img else 2) / init_height * 100
    ):
    ...
    soft_upscale = 1.0
    if (
        interface_width != init_width * (1 if img2img else 2)
        or interface_height != init_height
    ):
        soft_upscale = min(
            config["interface_width"] / init_width,
            config["interface_height"] / init_height,
        )
...
if (
    settings.get("override_settings", None) is not None
    and settings["override_settings"].get("CLIP_stop_at_last_layers", None) is not None
):
...
if settings.get("controlnet_units", None) and settings.get("controlnet_units")[0].get(
    "pixel_perfect", None
):
...
if (
    controlnet_model
    and settings.get("controlnet_units", None)
    and not settings.get("controlnet_units")[0].get("model", None)
):
    ...
    if index == "0":
        if preset_type == "render":
            presets[preset_type][index] = {
                "clip_skip": settings.get("override_settings", {}).get(
                    "CLIP_stop_at_last_layers", 1
                ),
                "hr_scale": config["hr_scales"][1]
                if settings.get("enable_hr", "false") == "true"
                else 1.0,
...
def load_preset(preset_type, index):
    ...
    # load preset
    for preset_field in (
        render_preset_fields if preset_type == "render" else cn_preset_fields
    ):
...
def load_filepath_into_canvas(file_path):
    ...
    pygame.draw.rect(
        canvas, (255, 255, 255), (0, 0, width * (1 if img2img else 2), height)
    )
        ...
        if m:
            if int(m.group(1)) >= autosave_images_max:
                ...
                rename_image(
                    ...
                    os.path.join(
                        file_path, f"{int(m.group(1))+1:02d}-{m.group(2)}.png"
                    ),
                )
...
def autosave_image(image_bytes):
    ...
    file_path = "outputs"
    if autosave_images_max > 0 and not os.path.exists(
        os.path.join(file_path, "autosave")
    ):
    ...
    if isinstance(image_bytes, list):
        ...
        for f in os.listdir(file_path):
            if not batch_image_pattern.match(f) or os.path.isdir(
            ...
            if autosave_images_max > 0:
                rename_image(
                    os.path.join(file_path, f),
                    os.path.join(file_path, "autosave", f"01-{f}"),
                )
        ...
        for i in range(len(image_bytes)):
            ...
            save_image(
                os.path.join(file_path, file_name), image_bytes[i], save_sketch=False
            )
    else:
        ...
        if (
            ...
            and autosave_images_max > 0
        ):
            rename_image(
                os.path.join(file_path, file_name),
                os.path.join(file_path, "autosave", f"01-{file_name}"),
            )
...
def update_image(image_data):
    ...
    if soft_upscale != 1.0:
        img_surface = pygame.transform.smoothscale(
            img_surface,
            (
                img_surface.get_width() * soft_upscale,
                img_surface.get_height() * soft_upscale,
            ),
        )
    ...
    for image_data in image_datas:
        ...
        if soft_upscale != 1.0:
            img_surface = pygame.transform.smoothscale(
                img_surface,
                (
                    img_surface.get_width() * soft_upscale // nb,
                    img_surface.get_height() * soft_upscale // nb,
                ),
            )
...
def select_batch_image(pos):
    ...
    for batch_image in batch_images:
        if (
            batch_image["coord"][0]
            <= pos[0]
            < batch_image["coord"][0] + batch_image["coord"][2]
            and batch_image["coord"][1]
            <= pos[1]
            < batch_image["coord"][1] + batch_image["coord"][3]
        ):
            need_redraw = True
...
def draw_osd_text(
    ...
    screen.blit(
        text_surface,
        (
            rect[0] + text_surface.get_width() * align_offset + distance,
            rect[1] + distance,
            rect[2],
            rect[3],
        ),
    ...
    screen.blit(
        text_surface,
        (rect[0] + text_surface.get_width() * align_offset, rect[1], rect[2], rect[3]),
    )
...
def osd(**kwargs):
    ...
    osd_margin = 10
    osd_progress_pos = (
        width * (0 if img2img else 1) + osd_margin,
        osd_margin,
    )  # top left of canvas
    ...
    osd_dot_pos = (
        width * (1 if img2img else 2) - osd_dot_size * 2 - osd_margin,
        osd_margin,
        osd_dot_size,
        osd_dot_size,
    )  # top right
    ...
    if rendering or (server_busy and progress is not None and progress < 0.02):
        ...
        pygame.draw.circle(
            rendering_dot_surface,
            (0, 0, 0),
            (osd_dot_size + 2, osd_dot_size + 2),
            osd_dot_size - 2,
        )
        pygame.draw.circle(
            rendering_dot_surface,
            (0, 200, 160),
            (osd_dot_size, osd_dot_size),
            osd_dot_size - 2,
        )
    ...
    if progress is not None and progress > 0.01:
        ...
        pygame.draw.rect(
            progress_surface,
            (0, 0, 0),
            pygame.Rect(2, 2, math.floor(osd_size[0] * progress), osd_size[1]),
        )
        pygame.draw.rect(
            progress_surface,
            (0, 200, 160),
            pygame.Rect(0, 0, math.floor(osd_size[0] * progress), osd_size[1] - 2),
        )
        ...
        draw_osd_text(
            f"{progress * 100:.0f}%",
            (
                osd_size[0] - osd_margin + osd_progress_pos[0],
                3 + osd_progress_pos[1],
                osd_size[0],
                osd_size[1],
            ),
            right_align=True,
    ...
    if osd_always_on_text:
        ...
        for line in osd_always_on_text.split("\n"):
            ...
            draw_osd_text(
                ...
                (
                    osd_text_pos[0],
                    osd_text_pos[1] + osd_text_offset,
                    osd_size[0],
                    osd_size[1],
                ),
            )
            if line_value:
                draw_osd_text(
                    line_value,
                    (
                        osd_text_pos[0] + osd_text_split_offset,
                        osd_text_pos[1] + osd_text_offset,
                        osd_size[0],
                        osd_size[1],
                    ),
...
def payload_submit():
    ...
    json_data["hr_second_pass_steps"] = max(
        8, math.floor(json_data["steps"] * denoising_strength)
    )  # at least 8 steps
...
def controlnet_to_sdapi(json_data):
    ...
    if not json_data["alwayson_scripts"].get("controlnet", {}):
        json_data["alwayson_scripts"]["controlnet"] = {"args": []}

    if json_data.get("controlnet_units", []) and not json_data.get(
        "alwayson_scripts", {}
...
def send_request():
    ...
    response = requests.post(
        url=f'{url}/sdapi/v1/{"img2img" if img2img else "txt2img"}',
        json=controlnet_to_sdapi(main_json_data),
    )
    if response.status_code == 200:
        r = response.json()
...
def brush_stroke(pos):
    ...
    if prev_pos is None or (
        abs(event.pos[0] - prev_pos[0]) < brush_size[button] // 4
        and abs(event.pos[1] - prev_pos[1]) < brush_size[button] // 4
    ):
        ...
        pygame.draw.polygon(
            canvas, brush_colors[button], [prev_pos, event.pos], brush_size[button] * 2
        )
        ...
        offset_pos_prev = [
            (brush_size[button] * angle_prev[3]),
            (brush_size[button] * angle_prev[2]),
        ]
    ...
    for field in fields:
        ...
        if "." in field:
            ...
            if isinstance(var, dict) and var.get(field[1], None) is not None:
                ...
                (isinstance(var, list) or isinstance(var, tuple))
                and field[1].isnumeric()
                and int(field[1]) < len(var)
            ):
                label = field[0]
...
class TextDialog(simpledialog.Dialog):
    ...
    def apply(self):
        if "_" + self.e1.get("1.0", tk.INSERT)[-1:] + "_" == "_\n_":
            p = self.e1.get("1.0", tk.INSERT)[:-1] + self.e1.get(
                tk.INSERT, tk.END
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            if event.pos[0] < width:
                ...
                if shift_down() and brush_pos[brush_key] is not None:
                    if shift_pos is None:
                        ...
                        pygame.draw.polygon(
                            canvas,
                            brush_colors[brush_key],
                            [shift_pos, brush_pos[brush_key]],
                            brush_size[brush_key] * 2,
                        )
                        shift_pos = brush_pos[brush_key]
            ...
            if event.key == pygame.K_UP:
                ...
                if ctrl_down():
                    with TextDialog(
                        seed, title="Seed", dialog_width=200, dialog_height=30
                    ) as dialog:
                        if dialog.result and dialog.result.isnumeric():
                            osd(text=f"Seed: {dialog.result}")
                            seed = int(dialog.result)
                            update_config(
                                json_file, write=autosave_seed, values={"seed": seed}
                            )
                            rendering = True
                            instant_render = True
                else:
                ...
                if ctrl_down():
                    ...
                    with TextDialog(negative_prompt, title="Negative prompt") as dialog:
                        if dialog.result:
                            ...
                            update_config(
                                json_file,
                                write=autosave_negative_prompt,
    ...
    screen.blit(
        cursor_surface,
        (mouse_pos[0] - cursor_size // 2, mouse_pos[1] - cursor_size // 2),
    )
